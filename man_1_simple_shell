...
.\"Created by Abel Bekele and Yohannes Geremew

.TH SODASH 1 "Aug 4 2022" "sila SHELL - Simple Shell man page"

.SH NAME
sila\fR \- simple UNIX command interpreter

.SH SYNOPSIS
sila\fR [\fIfilename\fR]

.SH COPYRIGHT
sila is Copyright (C) 2022 by Abel Bekele and Yohannes Geremew

.SH DESCRIPTION
Sila\fR is a sh-compatible command language interpreter that executes commands read from the standard input or from a file.
...
.SH INVOCATION
Sila is started with the standard input connected to the terminal (as determined by \fIisatty(3)\fR), the interactive mode is executed. Also non-interactively option can be executed.
...
.SH AUTHORS
Sebrina Teka and Michael Kebede
